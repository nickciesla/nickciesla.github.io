...
$(document).ready(function() {
  $.getJSON('https://site.api.espn.com/apis/site/v2/sports/golf/pga/scoreboard', function(data) {
    ...
    for (let i = 0; i < 88; i++) {
      ...
      scores[golfer] = [score, day1, day2, day3, day4]
    }
    ...
    for (let [name, golfers] of Object.entries(teams)) {
      ...
      for (let [golfer, score] of Object.entries(scores)) {
        ...
        if (golfers.includes(golfer)) {
          ...
          if (score_count < 4) {
            if (score[0] != 'E') {
              score_total = score_total + parseInt(score[0])
            }
            score_count = score_count + 1
            $("#teamscores").append('<div class="row text-center align-items-center"><div class="col-md text-truncate">' + golfer + '</div><div class="col-md">' + score + '</div></div>')
            wrote = true
          }
